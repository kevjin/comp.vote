...
export default function Header() {
  ...
  const pageTitle = `${router.pathname.startsWith('/delegate') ? 'Delegate' : 'Vote'} By Signature`
  if (embedded) {
  ...
  return (
    <div className={styles.header}>
      ...
      <h1 className={styles.notEmbeddedHeader}>{pageTitle}</h1>
    </div>
  );
}
